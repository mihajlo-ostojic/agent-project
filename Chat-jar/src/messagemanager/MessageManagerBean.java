...
public class MessageManagerBean implements MessageManagerRemote {
	...
	private Message createNewTextMessage(ACLMessage amsg) {
		Message msg = null ;
		try {
			msg = session.createTextMessage();
			for(String property : amsg.userArgs.keySet()) {
			...
			if(amsg.performative != null) {
			msg.setObjectProperty("performative", amsg.performative.toString());}
			if(amsg.isNotForChat)
			{
				msg.setObjectProperty("forchat", "yes");
			}else {
				msg.setObjectProperty("forchat", "no");
			}
			return msg;
		} catch (JMSException e) {
			// TODO Auto-generated catch block
	...
	public void post(ACLMessage msg) {
		try {
			defaultProducer.send(createNewTextMessage(msg));
		} catch (JMSException e) {
			e.printStackTrace();
